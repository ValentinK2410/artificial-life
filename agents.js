...
class Agent {
    ...
    updateTemperature() {
        ...
        const MIN_TEMP = -50; // Минимальная температура тела (°C, защита от ошибок)
        const MAX_TEMP = window.GAME_CONFIG?.AGENTS?.MAX_TEMPERATURE || 37; // Максимальная температура тела (°C)
        
        // Если температура вышла за разумные пределы - устанавливаем в окружающую среду
        if (this.temperature < MIN_TEMP || this.temperature > MAX_TEMP || isNaN(this.temperature)) {
            console.warn(`⚠️ Некорректная температура у агента ${this.name}: ${this.temperature}. Устанавливаем в окружающую среду.`);
