...
class Agent {
    constructor(name, age, gender, type, ownerId = null) {
        ...
        this.forceHeal = false; // Флаг принудительного лечения (самый высокий приоритет)
    ...
    checkForSickAgents() {
        ...
        // Если включен принудительный режим лечения, не сбрасываем больного агента
        if (!this.forceHeal) {
            this.sickAgent = null; // Сбрасываем больного агента только если не принудительное лечение
        }
        if (!window.agents || !window.agents.getAllAgents) return;
        ...
        const searchRadius = this.forceHeal ? 500 : 150; // Увеличиваем радиус поиска при принудительном лечении
        ...
        allAgents.forEach(agent => {
            ...
            if (agent.health < CRITICAL_HEALTH_THRESHOLD) {
                ...
                // Если больной агент в радиусе поиска и ближе предыдущего
    ...
    hasMedicalSupplies() {
        // Проверяем, есть ли у агента аптечка ИЛИ лечебные травы (достаточно одного из них)
    ...
    heal() {
        ...
        if (distance > 20) {
            // Далеко от больного - спешим к нему (уменьшен порог для более быстрого лечения)
            this.moveTo(this.sickAgent.position.x, this.sickAgent.position.y);
            // Блокируем другие задачи во время движения к больному
        ...
        // Используем траву или аптечку для лечения
        const FOOD_PROPERTIES = window.FOOD_PROPERTIES || {}; // Объект со свойствами всех видов еды
        let healthRestore = 0; // Количество восстанавливаемого здоровья
        let immunityRestore = 0; // Количество восстанавливаемого иммунитета
        
        // Если есть трава - используем её свойства
        if (herbItem) {
            const props = FOOD_PROPERTIES[herbItem.type]; // Свойства лечебной травы (объект {health, immunity, ...} или undefined)
            if (props) {
                healthRestore = props.health || 0; // Количество восстанавливаемого здоровья
                immunityRestore = props.immunity || 0; // Количество восстанавливаемого иммунитета
            }
        }
        
        // Если есть аптечка - добавляем бонус к лечению
        const hasFirstAidKit = this.inventory.some(item => item.type === 'first_aid_kit');
        if (hasFirstAidKit) {
            healthRestore += 5; // Аптечка дает дополнительное здоровье
        }
        
        // Если нет ни трав, ни аптечки - используем базовое лечение (очень слабое)
        if (healthRestore === 0) {
            healthRestore = 2; // Минимальное лечение без инструментов
            if (window.addLogEntry) {
                window.addLogEntry(`💊 ${this.name} пытается лечить ${this.sickAgent.name} без медицинских принадлежностей (эффективность низкая)`);
            }
        }
        
        // Если опыта лечения мало - уменьшаем эффективность
        const healingSkill = this.experience.healing || 0;
        if (healingSkill < 5) {
            healthRestore = Math.floor(healthRestore * 0.5); // При малом опыте лечение в 2 раза слабее
        }
        ...
        if (healthRestore > 0) {
                ...
                if (immunityRestore > 0) {
                    this.sickAgent.immunity = Math.min(100, (this.sickAgent.immunity || 0) + immunityRestore); // Восстанавливаем иммунитет (не выше 100)
                }
                
                // Убираем использованную траву из инвентаря или запасов еды (если была использована)
                if (herbItem) {
    ...
    decide() {
        ...
        // ========== ПРИНУДИТЕЛЬНОЕ ЛЕЧЕНИЕ (самый высокий приоритет) ==========
        if (this.forceHeal) {
        ...
        // Проверяем наличие больных агентов (высокий приоритет - до других задач)
        this.checkForSickAgents();
        if (this.sickAgent && this.hasMedicalSupplies()) {
            ...
            // Больной агент найден, но нет медицинских принадлежностей - ищем их
            const neededSupply = { type: 'medical', resourceTypes: ['first_aid_kit', 'rosehip', 'st_johns_wort', 'mint', 'honey'] };
    ...
    fish() {
        // Рыбалка - нужен навык, удочка и водоем
        if (!window.world) return;
        
        // Проверяем навык рыбалки (минимум 5 опыта)
        if (!this.experience.fishing || this.experience.fishing < 5) {
            if (window.addLogEntry && Math.random() < 0.2) {
                window.addLogEntry(`🎣 ${this.name} не умеет ловить рыбу (нужен навык рыбалки >= 5)`);
            }
            this.state = 'explore';
        ...
        // Проверяем, что агент находится достаточно близко к водоёму (максимальное расстояние для начала рыбалки)
        const maxDistanceForFishing = 100; // Максимальное расстояние от края водоёма для начала рыбалки
        if (distanceFromEdge > maxDistanceForFishing) {
            // Слишком далеко от водоёма - физически невозможно ловить рыбу
            if (window.addLogEntry && Math.random() < 0.2) {
                window.addLogEntry(`🎣 ${this.name} слишком далеко от водоёма, чтобы ловить рыбу`);
            }
            this.state = 'explore';
            this.targetPosition = null;
            return;
