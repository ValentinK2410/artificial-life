...
export const GAME_CONFIG = {
    ...
    AGENTS: {
        ...
        MIN_TEMPERATURE: 25, // Минимальная температура для выживания (снижено с 32 до 25)
        DEATH_TEMPERATURE: 20, // Критическая температура смерти от переохлаждения (снижено с 28 до 20)
        MAX_TEMPERATURE: 40, // Максимальная температура тела (увеличено с 37 до 40 для учета небольшого повышения от движения/костра)
        MAX_HUNGER: 100,
        ...
        // Настройки голода
        HUNGER: {
            INCREASE_RATE: 0.003,       // Скорость увеличения голода за обновление (еще больше уменьшено для баланса)
            CRITICAL_THRESHOLD: 85,     // Порог, когда начинает теряться здоровье (увеличен)
            HEALTH_LOSS_RATE: 0.05,     // Скорость потери здоровья при критическом голоде (уменьшено еще больше)
            AUTO_EAT_THRESHOLD: 40,     // Порог, когда агент начинает есть из запасов (уменьшен для более раннего питания)
            FOOD_RESTORE: 30,           // Сколько голода восстанавливает еда из запасов (увеличено)
            SEARCH_FOOD_THRESHOLD: 50,  // Порог, когда агент начинает искать еду (уменьшен для более раннего поиска)
            STORE_FOOD_THRESHOLD: 30,   // Порог, когда агент начинает запасать еду (уменьшен для более раннего запасания)
            WARNING_THRESHOLD: 85       // Порог для предупреждения о критическом голоде
        },
